...
void	printInstructions(void) {
	...
	std::cout 	<< printColor(LINE1_TEXT, LINE1_CLR) << RESET << std::endl
				...
				<< printColor(CMD1, CMD_CLR)
				<< printColor(CMD1_PROMPT, CMD_PROMPT_CLR) << std::endl
			 	<< "🔍 " << std::left << std::setw(WIDTH)
				<< printColor(CMD2, CMD_CLR) << printColor(CMD2_PROMPT, CMD_PROMPT_CLR)
				<< std::endl
		 		// << "🙏 " << printColor(CMD3, CMD_CLR) << printColor(CMD3_PROMPT, CMD_PROMPT_CLR) << std::endl;
				<< "🚪 " << std::left << std::setw(WIDTH)
				<< printColor(CMD4, CMD_CLR) << printColor(CMD4_PROMPT, CMD_PROMPT_CLR)
				<< std::endl;
